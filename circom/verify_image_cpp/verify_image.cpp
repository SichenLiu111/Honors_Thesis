...
void equal_grid_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void equal_grid_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 73;}
...
Fr_neq(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[0]); // line circom 11
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + 2];
// load src
Fr_div(&expaux[0],&circuitConstants[1],&signalValues[mySignalStart + 1]); // line circom 11
// end load src
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 1]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
ctx->componentMemory[coffset].templateName = "equal_grid";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 9; i++) {
std::string new_cmp_name = "is_equal"+ctx->generate_position_array(aux_dimensions, 2, i);
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[2]); // line circom 45
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((3 * Fr_toInt(&lvar[2])) + (1 * Fr_toInt(&lvar[3]))) + 0)]);
}
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((3 * Fr_toInt(&lvar[2])) + (1 * Fr_toInt(&lvar[3]))) + 9)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
isEqual_1_run(mySubcomponents[cmp_index_ref],ctx);
...
equal_grid_2_create(1,0,ctx,"main",0);
equal_grid_2_run(0,ctx);
}
